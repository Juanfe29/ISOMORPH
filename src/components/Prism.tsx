...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const FIRE = Math.max(0, Math.min(1, firePalette || 0));
    ...
    const fragment = /* glsl */ `
      ...
      uniform float uFirePalette;

      // FORGED IN FIRE palette: ink-black → oxblood → brick-ember → autumn-leaf → amber-flame
      vec3 fireColor(float t) {
        t = clamp(t, 0.0, 1.0);
        vec3 dark   = vec3(0.012, 0.027, 0.118);  // #03071e ink black
        vec3 ember  = vec3(0.612, 0.008, 0.031);  // #9d0208 oxblood
        vec3 red    = vec3(0.816, 0.000, 0.000);  // #d00000 brick ember
        vec3 orange = vec3(0.910, 0.365, 0.016);  // #e85d04 autumn leaf
        vec3 amber  = vec3(1.000, 0.729, 0.031);  // #ffba08 amber flame
        if (t < 0.35) return mix(dark,   ember,  t           / 0.35);
        if (t < 0.55) return mix(ember,  red,   (t - 0.35)  / 0.20);
        if (t < 0.75) return mix(red,    orange,(t - 0.55)  / 0.20);
        return               mix(orange, amber, (t - 0.75)  / 0.25);
      }
      ...
      void main(){
        ...
        if(uFirePalette > 0.0001){
          float lum = dot(col, vec3(0.2126, 0.7152, 0.0722));
          col = mix(col, fireColor(lum), uFirePalette);
        }
    ...
    return () => {
      ...
      if (suspendWhenOffscreen) {
        ...
        if(c.__prismIO) { c.__prismIO.disconnect(); delete c.__prismIO; }
      }
      if(gl.canvas.parentElement===container) container.removeChild(gl.canvas);
    };
  }, [height,baseWidth,animationType,glow,noise,offset?.x,offset?.y,scale,transparent,hueShift,colorFrequency,timeScale,hoverStrength,inertia,bloom,suspendWhenOffscreen,firePalette]);
